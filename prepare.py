...
DOOM_READOUT_TYPES=['DNa02','DNp09','MDN','MN9','DNp20','DNpe017']

def prepare(dataset='malecns_v1',readout_types=None,projection='viewport'):
    readout_types=DOOM_READOUT_TYPES if readout_types is None else list(readout_types)
    if projection not in ('viewport','ommatidial'):
        raise ValueError(f'Unknown retinal projection: {projection!r}')
    ...
    manifest={'dataset':dataset,'projection':projection,'neurons':len(nodes),'edges':len(pre),'synaptic_contacts':int(count.sum(dtype=np.uint64)),
    ...
    out=ROOT/'outputs/connectome_sim'/dataset;out.mkdir(parents=True,exist_ok=True)
