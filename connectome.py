"""Loss-accounted import of the retained MaleCNS v1.0 graph for DOOMFLY.
...
def normalize_nodes(dataset_id: str, frame, nt_frame=None):
    ...
    if dataset_id == "malecns_v1":
        ...
        if nt_frame is not None:
            ...
            nt_source = pd.Series(["source_consensus_prediction_or_ground_truth"] * len(frame))
        candidate_reason = np.where(retain, "assigned_neuronal_superclass", "unresolved_object")
    else:
    ...
    catalog = pd.DataFrame({
        ...
        "object_kind": np.where(nonneural, "non_neuronal", np.where(retain, "neuron_candidate", "unresolved_object")),
        "inclusion_reason": np.where(nonneural, "explicit_non_neuronal_annotation", candidate_reason),
        "quality": np.asarray(quality), "superclass": np.asarray(superclasses),
        "cell_type": np.asarray(types), "neurotransmitter": np.asarray(predicted),
...
def import_graph(dataset_id: str = "malecns_v1") -> dict:
    ...
    source_dir = ROOT / "connectome_data" / dataset_id
    output = source_dir / "normalized"
    output.mkdir(parents=True, exist_ok=True)
    lock_path = source_dir / "source.lock.json"
    hashes = {name: {"url": url, "bytes": (source_dir / name).stat().st_size,
                     "sha256": file_digest(source_dir / name)} for name, url in config["files"].items()}
    if lock_path.exists():
        locked = json.loads(lock_path.read_text())
        if any(locked[name]["sha256"] != info["sha256"] for name, info in hashes.items()):
            raise ValueError("Source files changed since the lock was created; use a new versioned dataset directory.")
    else:
    ...
    stats = {key: 0 for key in ["source_edge_rows", "retained_edge_rows", "excluded_edge_rows",
                                "source_synaptic_contacts", "retained_synaptic_contacts", "excluded_synaptic_contacts",
                                "retained_weight_one_edges", "retained_self_edges"]}
    ...
    with pa.OSFile(str(temporary), "wb") as sink, ipc.new_file(sink, schema) as writer:
        for number in range(reader.num_record_batches):
            ...
            pre, post, weights = [batch.column(batch.schema.get_field_index(c)).to_numpy(zero_copy_only=False) for c in edge_columns]
    ...
    report = {"dataset_id": dataset_id, "sex": config["sex"], "release": config["release"],
              "coverage": config["coverage"], "source_annotation_rows": len(catalog),
              "retained_neuron_candidates": len(nodes), "excluded_object_counts": catalog.loc[~catalog.retained].object_kind.value_counts().to_dict(),
              ...
              "neurotransmitter_counts": nodes.neurotransmitter.fillna("missing").astype(str).value_counts().to_dict(),
              "isolated_neurons": int(np.count_nonzero((incoming == 0) & (outgoing == 0))),
              "node_policy": config["node_policy"], "edge_policy": config["edge_policy"],
              "upstream_filters": config["upstream_filters"], "upstream_autapses_excluded": config["upstream_autapses_excluded"],
              "additional_edge_strength_threshold": None, "synaptic_weights_are_contact_counts": True,
              "source_hashes": hashes, "graph": stats,
...
def main():
    ...
    parser.add_argument("dataset", choices=["malecns_v1"], nargs="?", default="malecns_v1")
    args = parser.parse_args()
    report = import_graph(args.dataset)
    print(json.dumps({k: report[k] for k in ["dataset_id", "retained_neuron_candidates", "graph", "neural_dynamics_validated"]}, indent=2))
