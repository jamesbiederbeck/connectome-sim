...
def provenance(graph,build,assets):
    audit=json.loads((ROOT/'outputs/connectome_sim/audit/data-integrity.json').read_text())
    actual=sha(graph)
    ...
    return {'model_revision':build['model_revision'],'kernel':build,'graph_sha256':actual,
      ...
      'data_audit_sha256':sha(ROOT/'outputs/connectome_sim/audit/data-integrity.json')}
